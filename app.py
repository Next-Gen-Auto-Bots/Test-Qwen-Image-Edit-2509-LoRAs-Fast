...
def infer(
    ...
    elif lora_adapter == "Real PS":
        pipe.set_adapters(["light-restoration"], adapter_weights=[1.0])
    elif lora_adapter == "Qwen-lora-nsfw":
        pipe.set_adapters(["relight", "edit-skin"], adapter_weights=[0.4, 0.6])
    elif lora_adapter == "Jib":
        pipe.set_adapters(["multi-angle-lighting"], adapter_weights=[1.0])
    elif lora_adapter == "A2R_2509_Base":
...
with gr.Blocks(css=css, theme=steel_blue_theme) as demo:
    with gr.Column(elem_id="col-container"):
        ...
        with gr.Row(equal_height=True):
            ...
            with gr.Column():
                output_image = gr.Image(label="Output Image1", interactive=False, format="png", height=350)
                
                with gr.Row():
                    lora_adapter = gr.Dropdown(
                        label="Choose Editing Style",
                        choices=["QWEN_MCNL", "Multiple-Angles", "Real PS", "Jib", "V14-10", "Qwen-lora-nsfw", "Next-Scene", "A2R_2509_Base"],
                        value="QWEN_MCNL"
                    )
